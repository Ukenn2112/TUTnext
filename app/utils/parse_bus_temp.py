...
依赖: pip install pdfplumber requests
用法:
    python parse_bus_temp.py <URL> [URL2] ...
"""
...
# ── 主解析函数 ────────────────────────────────────────────────────────────────
def parse_temp_pdf(url: str, date: datetime_date | None = None) -> dict:
    """
    从 URL 流式读取临时时刻表 PDF，返回结构化时刻表 dict。
    ...
    if not tables:
        raise ValueError(f"未在 PDF 中找到表格：{url}")
