...
class GoogleClassroomAPI:
    ...
    async def revoke_user_authorization(self, username: str) -> Dict[str, Any]:
        ...
        if self.client_id is None:
            logging.warning("Google Classroom client_id is not configured; skipping revocation.")
            return {"success": False, "message": "Google Classroom is not configured", "error": "client_id is None"}
        try:
            # 从数据库获取用户令牌
            tokens = await db_manager.get_user_tokens(username)
            if not tokens:
